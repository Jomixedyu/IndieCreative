...
public class BuffMgr
{
    ...
    public void FixedUpdate(int frame)
    {
        ...
        foreach (var item in buffs)
        {
            if (frame >= item.EndTime)
            {
                if (deleteBuff == null)
                {
                    deleteBuff = JxUnity.Pools.ListPool.Get<BuffAbstract>();
                }
                deleteBuff.Add(item);
            }
        ...
        if (deleteBuff != null)
        {
            ...
            JxUnity.Pools.ListPool.Recycle(deleteBuff);
        }
    }
    ...
    {
        using var _deleteBuff = JxUnity.Pools.ListPool.Use<BuffAbstract>();
        var deleteBuff = _deleteBuff.Item;
